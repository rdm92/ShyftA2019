...
delayDays = WScript.Arguments.Item(1)
delayDays = CInt(delayDays)
...
For each item1 in objFolder.Items
  ...
  if item1.Unread=true then
    ...
    objReceiveDate = item1.ReceivedTime
    currentDate = now()
    DaysDiff = DateDiff("d",objReceiveDate,currentDate)
    if DaysDiff >= delayDays then
      ...
      If intcount > 0 Then
	     ...
	     AtchFileName = Replace(item1.Attachments(i).filename,".pdf","")
	     AtchFileName = AtchFileName & "_" & AtchmntCounter & ".pdf"
              If InStr(LCase(item1.Attachments(i).filename), ".pdf") Then
                t = now()
