      ...
      If intcount > 0 Then
         ...
         For i = 1 To intcount
            If InStr(LCase(item1.Attachments(i).filename), ".pdf") Then
               vFlagExcelAttachmentFound = True
            End if
         Next
         'File format check ENDS
         If vFlagExcelAttachmentFound = True Then
            For i = 1 To intcount
                         
              If InStr(LCase(item1.Attachments(i).filename), ".pdf") Then
                t = now()
                savename   = saveFolder & "\" & item1.Attachments(i).filename
                ...
                ts.WriteLine item1.Attachments(i).filename & "," & vSenderEmailAddress & "," &  item1.Subject
                ts.Close
